...
class SelectableGroup extends React.Component {
	...
	_selectElements (e) {
		...
		this._registry.forEach(itemData => {
			if(itemData.domNode && doObjectsCollide(selectbox, itemData.domNode, tolerance)) {
				newItems.push(itemData.key);
				if(this.props.selectedItems.indexOf(itemData.key) == -1 && dontClearSelection){
    ...
    newItems.forEach(item => {
      if (this.props.selectedItems.indexOf(item) === -1) {
      ...
      newCurrentItems = this.props.selectedItems.concat(shouldBeAddedArray);
			...
			newCurrentItems = this.props.selectedItems.filter(function(i) {return newItems.indexOf(i) < 0;}); // Delete newItems from currentItems
		}
